 ...
 */
function addNews() {
	document.getElementById("formNewsAdd").submit();
}

function updateNews(data) {
	var user_id = document.getElementById("form_update_news_id");
	user_id.value = data;
	document.getElementById("formNewsUpdate").submit();
}
        ...
        success: function(data){
       	 ...
       	 if( true == retDate.MANAGER_RESULT ) {
       		...
       		location.reload();  
       		 ...
       		 //执行失败，alert错误信息
       		 alert("删除农信要闻信息失败，失败原因：" + retDate.MANAGER_ERROR_MESSAGE );
       	 }
       }	
	}); 
}
...
<body>
	<form id="formNewsUpdate" name="formNewsUpdate" action="toNewsUpdate" >
		<input type="hidden" id="form_update_news_id" name="news.id"/>
	</form>
	<form id="formNewsAdd" name="formNewsAdd" action="toNewsAdd" />
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <li ><span><a href="javascript:addNews();"><img src="images/t01.png" /></span>添加</a></li>
        </ul>
    </div>
    ...
    <table class="tablelist">
        ...
        <tbody>
        <s:iterator value="newsList" var="newss">
            <tr>
	        <td><input name="" type="checkbox" value="" /></td>
	        <td>${ newss.id }</td>
	        <td>${ newss.title }</td>
	        <td>${ newss.author_name }</td>
	        <td>${ newss.create_time }</td>
	        <td>已审核</td>
	        <td><a href="javascript:updateNews('${ newss.id }')" class="tablelink" >更新</a>
	            <a href="javascript:deleteNews('${ newss.id }')" class="tablelink"> 删除</a></td>
	        </tr> 
    	</s:iterator> 
        </tbody>
    </table>
...
<script>
	...
	function pageGo() {
		...
		if( isNaN(goNumber) ||  goNumber <= 0 || goNumber > allPage ) {
			...
			window.location.href="toNewsList?page.number="+goNumber+"&page.count="+count+"";			
		}
	}
	...
	function goLastPage() {
		window.location.href="toNewsList?page.number=1&page.count="+count+"";
	}
	
	function goNextPage() {
		window.location.href="toNewsList?page.number="+allPage+"&page.count="+count+"";
	}
</script>
</html>
