 ...
 */
function addPhoto() {
	document.getElementById("formPhotoAdd").submit();
}

function deletePhoto( data ) {
	$.ajax({  
        url :"photoDelete?"+
       		 "photo.id="+data+"",//后台处理程序
        type:"post",    	//数据发送方式  
        async:false,  
        data:"",
        error: function(){  
        ...
        success: function(data){
       	 ...
       	 if( true == retDate.MANAGER_RESULT ) {
       		...
       		location.reload();  
       		 ...
       		 //执行失败，alert错误信息
       		 alert("删除图片失败，失败原因：" + retDate.MANAGER_ERROR_MESSAGE );
       	 }
       }	
	}); 
}
...
<body>
	...
	<form id="formPhotoAdd" name="formPhotoAdd" action="toPhotoAdd" />
    ...
    <div class="rightinfo">
	    ...
	    <div class="tools">
	    	...
	    	<ul class="toolbar">
	        <li ><span><a href="javascript:addPhoto();"><img src="images/t01.png" /></span>添加</a></li>
	        </ul>
	    </div>
	    ...
	    <table class="tablelist">
	        ...
	        <s:iterator value="photoList" var="photos">
		        ...
		        <td>${ photos.id }</td>
		        <td>${ photos.title }</td>
		        <td>${ photos.author_name }</td>
		        <td>${ photos.create_time }</td>
		        <td>已审核</td>
		        <td>
		            <a href="javascript:deletePhoto('${ photos.id }')" class="tablelink"> 删除</a></td>
		        </tr> 
	    	</s:iterator> 
	        </tbody>
	    </table>
...
<script>
	...
	function pageGo() {
		...
		if( isNaN(goNumber) ||  goNumber <= 0 || goNumber > allPage ) {
			...
			window.location.href="toPhotoList?page.number="+goNumber+"&page.count="+count+"";			
		}
	}
	...
	function goLastPage() {
		window.location.href="toPhotoList?page.number=1&page.count="+count+"";
	}
